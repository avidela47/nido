...
import type { BudgetMonthly, BudgetRow } from "../../lib/budgets";
import { formatCurrencyARS } from "../../lib/format";

function badge(status: BudgetRow["status"]) {
...
function label(status: BudgetRow["status"]) {
...
export default function BudgetsClient({ initial }: { initial: BudgetMonthly }) {
  const router = useRouter();
  const params = useSearchParams();

  const [data, setData] = useState<BudgetMonthly>(initial);
  ...
  async function refresh(nextMonth?: string) {
    ...
    const res = await fetch(`/api/budgets?month=${encodeURIComponent(m)}`, {
      cache: "no-store",
    });
    const json = await res.json().catch(() => null);
    if (json?.ok && json.data) setData(json.data as BudgetMonthly);
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        ...
        <div className="rounded-2xl border border-[rgb(var(--border))] bg-white px-4 py-3">
          <div className="text-xs text-[rgb(var(--subtext))]">Total presupuestado</div>
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
        {data.rows.map((row) => (
          <BudgetCard
            ...
            row={row}
            onSave={save}
            saving={savingId === row.categoryId}
          />
        ))}
      </div>
    </div>
    ...
    <div className="rounded-3xl border border-[rgb(var(--border))] bg-white p-4 shadow-[0_1px_0_rgba(15,23,42,0.04),0_12px_32px_rgba(15,23,42,0.06)]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="truncate text-sm font-semibold">{row.categoryName}</div>
          <div className="mt-1 text-xs text-[rgb(var(--subtext))]">
            Gastado:{" "}
            <span className="font-semibold">{formatCurrencyARS(-Math.abs(row.spent))}</span> ·
            Presupuesto:{" "}
            <span className="font-semibold">{formatCurrencyARS(row.budget)}</span>
          </div>
        </div>
      ...
      <div className="mt-3">
        ...
        <div className="mt-2 flex items-center justify-between text-xs text-[rgb(var(--subtext))]">
          <div>
            Avance: <span className="font-semibold">{row.budget > 0 ? Math.round(row.percent) : 0}%</span>
          </div>
          <div>
            Restante:{" "}
      ...
      <div className="mt-3 flex flex-col gap-2 md:flex-row md:items-center">
        <div className="flex-1">
          <div className="text-xs font-semibold text-[rgb(var(--subtext))]">Presupuesto (ARS)</div>
          <input
            value={draft}
        ...
        <button
          ...
          onClick={async () => {
            ...
            await onSave(row.categoryId, amount);
          }}
